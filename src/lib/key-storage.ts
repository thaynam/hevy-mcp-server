...
import { isValidHexKey, isValidLength } from "./crypto-utils.js";
...
export async function encryptApiKey(
	...
	// Validate encryption key is 64 hex chars (32 bytes for AES-256)
	if (!isValidHexKey(encryptionKeyHex, 32)) {
		throw new Error("Invalid encryption key: must be 64 hex characters (32 bytes)");
	}
	
	// Validate API key length
	if (!isValidLength(apiKey, 500)) {
