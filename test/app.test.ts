  ...
  return {
    createMcpRoutes: vi.fn((handlers: any) => {
      ...
      routes.all("/mcp", async (c) => {
        const response = await handlers.streamableHTTP.fetch(
          c.req.raw,
          c.env,
          c.executionCtx,
        );
      ...
      routes.all("/sse", async (c) => {
        const response = await handlers.sse.fetch(
          c.req.raw,
          c.env,
          c.executionCtx,
        );
...
describe("Hono App Integration", () => {
  ...
  describe("Error Handling", () => {
    it("should handle unhandled errors gracefully", async () => {
      ...
      mcpHandlers.streamableHTTP.fetch.mockRejectedValue(
  ...
  describe("Route Ordering", () => {
    ...
    it("should route MCP requests to MCP handlers", async () => {
      mcpHandlers.streamableHTTP.fetch.mockResolvedValue(
